...
const SYSTEM = `You are a research analyst summarising LinkedIn company post feedback into a platform-level brief.
...
Rules:
- platform must always be "linkedin" (exactly that string).
...
export interface LinkedInSummarizeInput {
  ctx: PipelineCtx;
  extract: PlatformExtract;
}

export function buildLinkedInSummarize(input: LinkedInSummarizeInput): {
  system: string;
  user: string;
