...
const SYSTEM = `You are a research analyst summarising Medium blog opinions into a platform-level brief.
...
- Focus on long-form practitioner opinions and analytical critiques, not casual comments.
- Never invent data not present in the extract; omit rather than fabricate.`;

export interface MediumSummarizeInput {
  ctx: PipelineCtx;
  extract: PlatformExtract;
}

export function buildMediumSummarize(input: MediumSummarizeInput): {
  system: string;
  user: string;
