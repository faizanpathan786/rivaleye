...
const SYSTEM = `You are a research analyst summarising Google Play Store feedback into a platform-level brief.
...
export interface PlayStoreSummarizeInput {
  ctx: PipelineCtx;
  extract: PlatformExtract;
}

export function buildPlayStoreSummarize(input: PlayStoreSummarizeInput): {
  system: string;
  user: string;
