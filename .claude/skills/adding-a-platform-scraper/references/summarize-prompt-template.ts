...
const SYSTEM = `You are a research analyst summarising <PLATFORM> feedback into a platform-level brief.
...
export interface ExampleSummarizeInput {
  ctx: PipelineCtx;
  extract: PlatformExtract;
}

export function buildExampleSummarize(input: ExampleSummarizeInput): {
  system: string;
  user: string;
